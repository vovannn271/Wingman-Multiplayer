...
namespace HeathenEngineering.SteamworksIntegration
{
    ...
    {
        public List<ItemDetail> Details => details;
        public long TotalQuantity => details.Sum(p => (long)p.Quantity);
        /// <summary>
        ...
        public string DisplayName
        {
            get
            {
                return API.Inventory.Client.GetItemDefinitionProperty(Id, "name");
            }
        }
        ...
        public bool HasPrice
        {
            get
            {
                return API.Inventory.Client.GetItemPrice(Id, out ulong _, out ulong _);
            }
        }
        ...
        public ulong CurrentPrice
        {
            get
            {
                if (API.Inventory.Client.GetItemPrice(Id, out ulong current, out ulong _))
                {
                    return current;
                }
                else
        ...
        public ulong BasePrice
        {
            get
            {
                if (API.Inventory.Client.GetItemPrice(Id, out ulong _, out ulong baseprice))
                {
                    return baseprice;
                }
                else
        ...
        [NonSerialized]
        private readonly List<ItemDetail> details = new List<ItemDetail>();
        ...
        public class Bundle
        {
            ...
            public struct Entry
            {
                ...
                public override string ToString()
                {
                    ...
                    {
                        if (count > 0)
                            return item.item_itemdefid.ToString() + "x" + count.ToString();
                        else
                            return item.item_itemdefid.ToString();
                    }
                }
            }
        ...
        {
            get => new SteamItemDef_t(item_itemdefid);
#if UNITY_EDITOR
            set
            {
                item_itemdefid = value.m_SteamItemDef;
            }
#endif
        }
        [SerializeField]
        internal int item_itemdefid;
        ...
        public ItemDefinition[] TagGenerators => item_tag_generators.ToArray();
        [SerializeField]
        internal List<ItemDefinition> item_tag_generators = new List<ItemDefinition>();
        ...
        private string ItemString()
        {
            ...
            sb.Append("\n\t\t\"itemdefid\": " + item_itemdefid.ToString());
            sb.Append(",\n\t\t\"type\": \"item\"");
            sb.Append(",\n" + item_name.ToString());
            ...
            if (item_tag_generators.Count > 0)
            {
                ...
                for (int i = 0; i < item_tag_generators.Count; i++)
                {
                    ...
                    tg.Append(item_tag_generators[i].item_itemdefid);
                }
        ...
        /// <returns></returns>
        public bool AddPromoItem(Action<InventoryResult> callback) => API.Inventory.Client.AddPromoItem(Id, callback);
        ...
        public void Exchange(IEnumerable<ExchangeEntry> recipeEntries, Action<InventoryResult> callback)
        {
            ...
            API.Inventory.Client.ExchangeItems(Id, instances, counts, callback);
        }
        /// <summary>
        ...
        {
            API.Inventory.Client.GenerateItems(new SteamItemDef_t[] { Id }, new uint[] { 1 }, callback);
        }
        /// <summary>
        ...
        {
            API.Inventory.Client.GenerateItems(new SteamItemDef_t[] { Id }, new uint[] { quantity }, callback);
        }
        /// <summary>
        ...
        {
            API.Inventory.Client.StartPurchase(new SteamItemDef_t[] { Id }, new uint[] { 1 }, callback);
        }
        /// <summary>
        /// Starts the purchase process for the user, given a shopping cart of item definitons that the user would like to buy
        /// </summary>
        ...
        {
            API.Inventory.Client.StartPurchase(new SteamItemDef_t[] { Id }, new uint[] { count }, callback);
        }
        /// <summary>
        ...
        /// <returns></returns>
        public bool GetPrice(out ulong currentPrice, out ulong basePrice) => API.Inventory.Client.GetItemPrice(Id, out currentPrice, out basePrice);
        /// <summary>
        /// Trigger an item drop if the user has played a long enough period of time.
        /// </summary>
        /// <param name="callback"></param>
        public void TriggerDrop(Action<InventoryResult> callback) => API.Inventory.Client.TriggerItemDrop(Id, callback);
        ...
        /// <param name="entries"></param>
        /// <returns></returns>
        public bool CanExchange(ExchangeRecipe recipie, out List<ExchangeEntry> entries)
        {
            if (!recipie.Valid)
            {
                entries = null;
...
#if UNITY_EDITOR
    [UnityEditor.CustomEditor(typeof(ItemDefinition))]
    public class ItemDefinitionEditor : UnityEditor.Editor
    {
        ...
        private UnityEditor.SerializedProperty item_itemdefid;
        ...
        private void OnEnable()
        {
            ...
            item_itemdefid = serializedObject.FindProperty("item_itemdefid");
        ...
        {
            var itemRef = target as ItemDefinition;
            ...
            UnityEditor.EditorGUILayout.PropertyField(item_itemdefid, new GUIContent("Id", "The ID of this itemdef. This must be lower than 1,000,000 for non-workshop items."), true);
