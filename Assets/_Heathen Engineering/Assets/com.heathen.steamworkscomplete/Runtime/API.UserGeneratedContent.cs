...
#region Workshop System
            public static bool CreateItem(WorkshopItemData item, Action<WorkshopItemDataCreateStatus> callback = null)
            {
                ...
                m_CreatedItem.Set(call, (createResult, createIOError) =>
                {
                    ...
                    { 
                        if (createIOError)
                        {
                            callback?.Invoke(new WorkshopItemDataCreateStatus
                            {
                                hasError = true,
                    ...
                    else
                    {
                        ...
                        if (!SteamUGC.SetItemContent(updateHandle, item.contentFolder))
                        {
                            hasError = true;
                            if (sb.Length > 0)
                                sb.Append("\n");
                        ...
                        if (!SteamUGC.SetItemPreview(updateHandle, item.previewImageFile))
                        {
                            hasError = true;
                            if (sb.Length > 0)
                                sb.Append("\n");
                        ...
                        {
                            foreach (var previewFile in item.previewFiles)
                            {
                        ...
                        if (item.youTubeIds != null && item.youTubeIds.Length > 0)
                        {
                            foreach (var video in item.youTubeIds)
                            {
                        ...
                        if (item.keyValueTags != null && item.keyValueTags.Length > 0)
                        {
                            foreach (var tag in item.keyValueTags)
                            {
                            ...
                            if (updateIOError || updateResult.m_eResult != EResult.k_EResultOK)
                            {
                                ...
                                if (updateIOError)
                                {
                                    ...
                                    callback?.Invoke(new WorkshopItemDataCreateStatus
                                    {
                                        ...
                                        ugcFileId = createResult.m_nPublishedFileId,
                                        createItemResult = createResult,
