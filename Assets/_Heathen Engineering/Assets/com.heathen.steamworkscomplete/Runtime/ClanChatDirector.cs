...
namespace HeathenEngineering.SteamworksIntegration
{
    public class ClanChatDirector : MonoBehaviour
    {
        ...
        public void Join(Clan clan)
        {
            API.Clans.Client.JoinChatRoom(clan, (result, error) =>
            {
                if (!error)
                    ...
                    Debug.LogWarning("Steam client responded with an IO error when attempting to join Clan chat for " + clan.id.ToString());
            });
        }
        ...
        private void HandleJoined(ChatRoom arg0, CSteamID arg1)
        {
