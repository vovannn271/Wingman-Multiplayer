...
namespace MD_PluginEditor
{
    ...
    public class MDM_SculptingLite_Editor : MD_EditorUtilities
    {
        private MDM_SculptingLite ss;

        private void OnEnable()
        {
            ss = (MDM_SculptingLite)target;
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (ss.SS_MeshSculptMode == MDM_SculptingLite.SS_MeshSculptMode_Internal.CustomDirection)
                ppDrawProperty("SS_CustomDirection", "Custom Direction", "Choose a custom direction in world space");
            else if (ss.SS_MeshSculptMode == MDM_SculptingLite.SS_MeshSculptMode_Internal.CustomDirectionObject)
            {
                    ...
                    if (ss.SS_UseInput)
                    {
                        ...
                        if (!ss.SS_VRInput)
                        {
                            ...
                            if (ss.SS_UseSmoothFunct)
                            {
                                ...
                                if (ss.SS_smoothType == MDM_SculptingLite.SS_SmoothType.LaplacianFilter) ppDrawProperty("SS_SmoothIntensity", "Smooth Intensity");
                                pve();
                            }
        ...
        private void OnSceneGUI()
        {
            ...
            if (Physics.Raycast(ray, out hit))
            {
                if (hit.collider == ss.transform.GetComponent<Collider>())
                {
                    ...
                    switch (ss.SS_State)
                    {
                        case MDM_SculptingLite.SS_State_Internal.Raise:
                            ss.SS_Funct_DoSculpting(hit.point, ss.SS_BrushProjection.transform.forward, ss.SS_BrushSize, ss.SS_BrushStrength, MDM_SculptingLite.SS_State_Internal.Raise);
                            if (!ss.SS_UpdateColliderAfterRelease)
                                ss.SS_Funct_RefreshMeshCollider();
                            break;
                        case MDM_SculptingLite.SS_State_Internal.Lower:
                            ss.SS_Funct_DoSculpting(hit.point, ss.SS_BrushProjection.transform.forward, ss.SS_BrushSize, ss.SS_BrushStrength, MDM_SculptingLite.SS_State_Internal.Lower);
                            if (!ss.SS_UpdateColliderAfterRelease)
                                ss.SS_Funct_RefreshMeshCollider();
                            break;

                        case MDM_SculptingLite.SS_State_Internal.Revert:
                            ss.SS_Funct_DoSculpting(hit.point, ss.SS_BrushProjection.transform.forward, ss.SS_BrushSize, ss.SS_BrushStrength, MDM_SculptingLite.SS_State_Internal.Revert);
                            if (!ss.SS_UpdateColliderAfterRelease)
                                ss.SS_Funct_RefreshMeshCollider();
                            break;
                    }
                ...
                if (Event.current.type == EventType.MouseDown && Event.current.button == 0 && !Event.current.alt)
                {
                    ...
                    if (!Event.current.control)
                    {
                        if (!Event.current.shift)
                            ss.SS_State = MDM_SculptingLite.SS_State_Internal.Raise;
                        else
                            ss.SS_State = MDM_SculptingLite.SS_State_Internal.Lower;
                    }
                    else
                        ss.SS_State = MDM_SculptingLite.SS_State_Internal.Revert;
                }
                else if (Event.current.type == EventType.MouseUp && Event.current.button == 0)
                {
                    ss.SS_State = MDM_SculptingLite.SS_State_Internal.None;
